...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def always[T](value: T): Future[T] = {
      val p = Promise[T]()
      p.success(value) // value 이용해서 성공 상태로 만들기
      p.future
    }
    ...
    def never[T]: Future[T] = {
      val p = Promise[T]()
      p.future // success 안했으니깐 무한 대기 상태?
    }
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      def addFutureResults[T](f: Future[T], acc: Future[List[T]]): Future[List[T]] = {
        for {
          future_result <- f
          past_results <- acc
        } yield future_result :: past_results
      }

      val initial_list = Promise[List[T]]()
      initial_list.success(Nil)
      fs.foldRight(initial_list.future)(addFutureResults)
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    //revised -> "taking the current future"
     ...
     *  The resulting future contains a value returned by `cont`.
     */
    //revised -> "taking the result of the current future"
