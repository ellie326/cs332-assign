...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    def asyncWork(ct: CancellationToken, listener: Listener, subscription: Subscription): Future[Unit] = {
      val p = Promise[Unit]()
      async {
    ...
    val new_listener = createListener(relativePath)
    val subscription = new_listener.start()

    Future.run() { ct => asyncWork(ct, new_listener, subscription) }
  }
}
