...
/**
 * @param elem = current tweet
 * @param left = left sub tree with smaller re-tweet #
 * @param right = right sub tree with more re-tweet #
 */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val filteredRight = right.filterAcc(p, filteredLeft)
    if (p(elem)) filteredRight.incl(elem)
    else filteredRight
  }

  def union(that: TweetSet): TweetSet = {
    val leftUnion = left.union(that)
  ...
  def mostRetweeted: Tweet = {
    if (!left.isEmpty && left.mostRetweeted.retweets > elem.retweets) left.mostRetweeted
